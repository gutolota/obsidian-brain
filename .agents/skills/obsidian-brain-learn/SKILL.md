...
- **Check for duplicates**: read `~/.agents/obsidian-brain/brain-rules.md` and compare with existing entries under `## Learned Rules`. If similar, propose refining the existing rule instead of adding a duplicate.
...
Open `~/.agents/obsidian-brain/brain-rules.md` and add to the `## Learned Rules` section:
...
Edit ~/.agents/obsidian-brain/brain-rules.md to adjust manually.
```
