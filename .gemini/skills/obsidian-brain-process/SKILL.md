...
For each rule in `~/.gemini/obsidian-brain/brain-rules.md` under `## Learned Rules`, check if it applies to this session and execute it. Common patterns:
...
1. Open `~/.gemini/obsidian-brain/brain-rules.md`
...
After the summary, show this line — **never run `/compress` yourself**:

```
🧠 Session saved to your vault.

To compress context, run: /compress
```

That's it. One line. Do not ask "would you like me to…" — just show it and let the user decide.
