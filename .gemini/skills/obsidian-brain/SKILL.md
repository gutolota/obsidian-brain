...
1. `~/.gemini/obsidian-brain/config.md` → vault path, folders, preferences
2. `~/.gemini/obsidian-brain/brain-rules.md` → learned rules (cumulative — respect all)
...
### process
Same as the dedicated `obsidian-brain:process` skill — deeper extraction, full daily note update, then **suggest** `/compress` for context compression. Never run it — just show it.
